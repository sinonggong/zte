...
module tb_axi_stripe;
    ...
    longint rq_a [$]; int rq_n [$];
    longint wq_a [$]; int wq_n [$];
    int     b_pending = 0;
    always @(posedge clk) begin
        ...
        if (m_arvalid && m_arready) begin
            ...
            rq_a.push_back(longint'(m_araddr >> 5)); rq_n.push_back(int'(m_arlen) + 1);
        end
        m_awready <= ($urandom_range(3, 0) != 0);
        if (m_awvalid && m_awready) begin
            if (!burst_ok(m_awaddr, m_awlen)) begin errors++; $display("BAD AW %h len %0d", m_awaddr, m_awlen); end
            wq_a.push_back(longint'(m_awaddr >> 5)); wq_n.push_back(int'(m_awlen) + 1);
        end
        m_wready <= ($urandom_range(2, 0) != 0) && (wq_a.size() > 0);
        if (m_wvalid && m_wready) begin
            pmem[wq_a[0]] = m_wdata;
            if ((wq_n[0] == 1) != m_wlast) begin errors++; $display("WLAST at the wrong beat"); end
            if (wq_n[0] == 1) begin void'(wq_a.pop_front()); void'(wq_n.pop_front()); b_pending++; end
            else begin wq_a[0] = wq_a[0] + 1; wq_n[0] = wq_n[0] - 1; end
        end
        if (m_bvalid && m_bready) m_bvalid <= 1'b0;
        else if (!m_bvalid && b_pending > 0 && ($urandom_range(2, 0) == 0)) begin m_bvalid <= 1'b1; b_pending--; end
        if (!m_rvalid || m_rready) begin
            if (rq_a.size() > 0 && ($urandom_range(3, 0) != 0)) begin
                m_rvalid <= 1'b1;
                m_rdata  <= pmem.exists(rq_a[0]) ? pmem[rq_a[0]] : 256'hDEAD;
                m_rlast  <= (rq_n[0] == 1);
                if (rq_n[0] == 1) begin void'(rq_a.pop_front()); void'(rq_n.pop_front()); end
                else begin rq_a[0] = rq_a[0] + 1; rq_n[0] = rq_n[0] - 1; end
    ...
    initial begin
        ...
        $display("RESULT %s tb_axi_stripe errors=%0d bursts=%0d B=%0d RLAST=%0d beats=%0d", errors == 0 ? "PASS" : "FAIL",
                 errors, NB, n_b, n_rlast, n_rbeats);
        $finish;
    end
    initial begin #20000000; $display("RESULT FAIL timeout (B %0d RLAST %0d)", n_b, n_rlast); $finish; end
endmodule
