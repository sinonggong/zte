#!/usr/bin/env bash
# Bit-exact Verilator run of paper/rtl/mlp72_int8_colpar_chain.sv against the
# behavioural ACX_MLP72 / ACX_BRAM72K models (paper/rtl/sim_models/) and the numpy
# golden (paper/sw/mlp72_colpar_golden.py).
#
# Usage: run_colpar_sim.sh <N_STAGE> <WORDS_PER_ROW> [ADDR_CASCADE=0] [SEED=1] [INROW_GAPS=1]
#        run_colpar_sim.sh all        # N_STAGE 1 4 16 x words 1 3 32 x ADDR_CASCADE 0 1
# Env:   MODEL_DEFS="+define+..."  flip a model assumption
#        CHAIN_RTL=<file>           simulate another RTL with the same ports
#        BUILD=<dir>                default build/paper_colpar_sim
set -euo pipefail
REPO=$(cd "$(dirname "$0")/../.." && pwd)
VERILATOR=${VERILATOR:-$HOME/tools/verilator5/bin/verilator}
BUILD=${BUILD:-$REPO/build/paper_colpar_sim}
RTL=${CHAIN_RTL:-$REPO/paper/rtl/mlp72_int8_colpar_chain.sv}
DEFS=${MODEL_DEFS:-}
...
TB=$REPO/paper/rtl/tb_mlp72_int8_colpar_chain.sv
TAG=$(echo "$DEFS|$RTL" | md5sum | cut -c1-8)
OBJ=$BUILD/obj_n${N}_c${C}_$TAG
VEC=$BUILD/vec_n${N}_w${W}_s${SEED}
EXE=$OBJ/Vtb_mlp72_int8_colpar_chain
mkdir -p "$BUILD"

python3 "$REPO/paper/sw/mlp72_colpar_golden.py" --n-stage "$N" --words "$W" --seed "$SEED" --out "$VEC" > "$VEC.log"
...
if [ $rebuild = 1 ]; then
    ...
    "$VERILATOR" --binary --timing -j 4 -O2 -Wno-fatal -Wno-lint -Wno-style -Wno-PINMISSING \
        -Wno-MULTIDRIVEN -Wno-UNOPTFLAT --top-module tb_mlp72_int8_colpar_chain \
